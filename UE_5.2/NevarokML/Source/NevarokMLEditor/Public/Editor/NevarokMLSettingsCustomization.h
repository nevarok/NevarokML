...
class FNevarokMLSettingsCustomization : public IDetailCustomization
{
public:
    static TSharedRef<IDetailCustomization> MakeInstance()
    {
        return MakeShareable(new FNevarokMLSettingsCustomization);
    }
    ...
    virtual void CustomizeDetails(IDetailLayoutBuilder& detailBuilder) override
    {
        ...
        IDetailCategoryBuilder& pathsCategory = detailBuilder.EditCategory("Paths", FText::FromString("Paths"));
        IDetailCategoryBuilder& installCategory = detailBuilder.EditCategory(
            "Installation", FText::FromString("Installation"));

        installCategory.AddCustomRow(FText::GetEmpty())
                       .ValueContent()
                       .HAlign(HAlign_Left)
        [
            SNew(SButton).HAlign(HAlign_Center)
                         .Text(FText::FromString("Refresh Details"))
        ...
        installCategory.AddCustomRow(FText::GetEmpty())
            ...
            [
                ...
                .Text(FText::FromString("Open Source Folder"))
                .OnClicked_Lambda([settings]() -> FReply
                {
                    FPlatformProcess::ExploreFolder(*settings->SourceFolderPath.Path);
                    return FReply::Handled();
                })
            ];
        ...
        [
            ...
            .Text(FText::FromString(
                                "Warning: In case of backend file issues, delete the executable folder and manually unpack\nthe backend archive or use the \"Unpack Backend\" option."))
        ...
        [
            SNew(STextBlock)
            .Text(FText::FromString(settings->BackendArchivePath.FilePath))
        ];

        if (!FPaths::FileExists(settings->BackendArchivePath.FilePath))
        {
            installCategory.AddCustomRow(FText::GetEmpty())
                           .ValueContent()
                           .HAlign(HAlign_Left)
            [
                SNew(STextBlock)
                .Text(FText::FromString("Error: Archive Not Found. Please try reimporting the plugin."))
        ...
        [
            SNew(STextBlock)
                    .Text(FText::FromString("OK: Backend Archive Found"))
        ...
        if (FPaths::FileExists(settings->BackendExePath.FilePath))
        {
            ...
            [
                SNew(STextBlock)
                .Text(FText::FromString(settings->BackendExePath.FilePath))
            ];
            installCategory.AddCustomRow(FText::GetEmpty())
                           .ValueContent()
                           .HAlign(HAlign_Left)
            [
                SNew(STextBlock)
                        .Text(FText::FromString("OK: Backend Executable Found"))
                        .ColorAndOpacity(FSlateColor(FLinearColor::Green))
            ];
            return;
        }

        if (FPaths::DirectoryExists(settings->BackendFolderPath.Path))
        {
            if (FPaths::FileExists(settings->DefaultBackendExePath.FilePath))
            {
                installCategory.AddCustomRow(FText::GetEmpty())
                               .ValueContent()
                               .HAlign(HAlign_Left)
                [
                    SNew(STextBlock)
                    .Text(FText::FromString(settings->DefaultBackendExePath.FilePath))
                ];
                installCategory.AddCustomRow(FText::GetEmpty())
                               .ValueContent()
                               .HAlign(HAlign_Left)
                ...
                settings->BackendExePath = settings->DefaultBackendExePath;
                settings->SaveConfig();
                detailBuilder.ForceRefreshDetails();
            ...
            [
                SNew(STextBlock)
                .Text(FText::FromString(settings->BackendFolderPath.Path))
            ];
            installCategory.AddCustomRow(FText::GetEmpty())
                           .ValueContent()
                           .HAlign(HAlign_Left)
            [
                SNew(STextBlock)
                        .Text(FText::FromString(
                                    "Error: Directory exists, but the executable is missing. Remove the directory, click \"Refresh Details,\"\nand proceed with \"Unpack Backend\" only if no warnings or errors are encountered."))
                        .ColorAndOpacity(FSlateColor(FLinearColor::Red))
            ];
            return;
        }
        ...
        [
            SNew(STextBlock)
                    .Text(FText::FromString(
                                "Error: Backend Executable Not Found. Please press the \"Unpack Backend\" button to install\nthe required executable."))
                    .ColorAndOpacity(FSlateColor(FLinearColor::Red))
        ];
        ...
        [
            SNew(SButton).HAlign(HAlign_Center)
                         .Text(FText::FromString("Unpack Backend"))
                         .OnClicked_Lambda([settings, &detailBuilder]() -> FReply
                         {
                             if (FPaths::DirectoryExists(settings->BackendFolderPath.Path))
                             {
                                 return FReply::Handled();
                             ...
                             UNevarokMLRunUtils::RunUnzip(settings->BackendArchivePath.FilePath,
                                                          settings->SourceFolderPath.Path);
