...
static void LexFromString(FNevarokMLIndex2D& outStruct, const TCHAR* inString)
{
    UScriptStruct* scriptStruct = FNevarokMLIndex2D::StaticStruct();
    FMemory::Memzero(&outStruct, sizeof(FNevarokMLIndex2D));

    scriptStruct->ImportText(inString,
                             &outStruct,
                             nullptr,
                             PPF_UseDeprecatedProperties,
                             reinterpret_cast<FOutputDevice*>(GWarn),
                             []() { return FString("FNevarokMLIndex2D"); });
}

static FString LexToString(const FNevarokMLIndex2D& inStruct)
{
    const UScriptStruct* scriptStruct = FNevarokMLIndex2D::StaticStruct();
    FString result;
    scriptStruct->ExportText(result, &inStruct, nullptr, nullptr, PPF_UseDeprecatedProperties, nullptr);

    return result;
}

class SNevarokMLIndex2DGraphPin : public SGraphPin
{
public:
    SLATE_BEGIN_ARGS(SNevarokMLIndex2DGraphPin)
        {
        }
    ...
    FNevarokMLIndex2D GetDefaultAsStruct() const
    {
        FNevarokMLIndex2D result;
        const auto str = GraphPinObj->GetDefaultAsString();
        LexFromString(result, *str);
        return result;
    }

    void SetPinValue(const FNevarokMLIndex2D& value) const
    {
        ...
        GraphPinObj->Modify();
        GraphPinObj->GetSchema()->TrySetDefaultValue(*GraphPinObj, value.ToString());
    }

    TOptional<int> GetXValue() const
    {
        return GetDefaultAsStruct().DX;
    }

    TOptional<int> GetYValue() const
    {
        return GetDefaultAsStruct().DY;
    }

    void SetXValue(const int32 x, ETextCommit::Type commitInfo) const
    {
        const auto defaultStruct = GetDefaultAsStruct();

        if(x == defaultStruct.DX)
        {
            return;
        }
        
        SetPinValue(FNevarokMLIndex2D(x, defaultStruct.DY));
    }

    void SetYValue(const int32 y, ETextCommit::Type commitInfo) const
    {
        const auto defaultStruct = GetDefaultAsStruct();

        if(y == defaultStruct.DY)
        {
            return;
        }
        
        SetPinValue(FNevarokMLIndex2D(defaultStruct.DX, y));
    }
    ...
    FSlateColor GetXTextBlockColor() const
    {
        return GetDefaultAsStruct().DX >= 0 ? FSlateColor(FLinearColor::Green) : FSlateColor(FLinearColor::Red);
    }

    FSlateColor GetYTextBlockColor() const
    {
        return GetDefaultAsStruct().DY >= 0 ? FSlateColor(FLinearColor::Green) : FSlateColor(FLinearColor::Red);
    }

    virtual TSharedRef<SWidget> GetDefaultValueWidget() override
    {
        return SNew(SBox).Visibility(IsConnected() ? EVisibility::Collapsed : EVisibility::Visible)
        [
            SNew(SHorizontalBox)
            + SHorizontalBox::Slot()
              .AutoWidth().Padding(2).VAlign(VAlign_Center)
            [
                SNew(STextBlock)
                .Text(FText::FromString(TEXT("dx:")))
                .ColorAndOpacity(this, &SNevarokMLIndex2DGraphPin::GetXTextBlockColor)
            ]
            + SHorizontalBox::Slot()
              .AutoWidth().Padding(2).HAlign(HAlign_Fill)
            [
                SNew(SNumericEntryBox<int32>)
                .Value(this, &SNevarokMLIndex2DGraphPin::GetXValue)
                .OnValueCommitted(this, &SNevarokMLIndex2DGraphPin::SetXValue)
            ]
            + SHorizontalBox::Slot()
              .AutoWidth().Padding(2).VAlign(VAlign_Center)
            [
                SNew(STextBlock)
                .Text(FText::FromString(TEXT("dy:")))
                .ColorAndOpacity(this, &SNevarokMLIndex2DGraphPin::GetYTextBlockColor)
            ]
            + SHorizontalBox::Slot()
              .AutoWidth().Padding(2).HAlign(HAlign_Fill)
            [
                SNew(SNumericEntryBox<int32>)
                .Value(this, &SNevarokMLIndex2DGraphPin::GetYValue)
                .OnValueCommitted(this, &SNevarokMLIndex2DGraphPin::SetYValue)
            ]
        ];
    }
};

struct FNevarokMLIndex2DGraphPinFactory : public FGraphPanelPinFactory
{
    virtual TSharedPtr<SGraphPin> CreatePin(UEdGraphPin* inPin) const override
    {
        if (inPin->PinType.PinCategory == UEdGraphSchema_K2::PC_Struct)
        {
            if (inPin->PinType.PinSubCategoryObject == FNevarokMLIndex2D::StaticStruct())
            {
                if (inPin->IsDefaultAsStringEmpty())
                {
                    inPin->DefaultValue = FNevarokMLIndex2D::NegativeOne().ToString();
                }

                inPin->AutogeneratedDefaultValue = inPin->DefaultValue;

                return SNew(SNevarokMLIndex2DGraphPin, inPin);
            }
        }
        return nullptr;
    }
