...
#pragma once
#include "NevarokMLIndex2D.generated.h"

USTRUCT(BlueprintType)
struct NEVAROKML_API FNevarokMLIndex2D
{
    GENERATED_BODY()

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="NevarokML|Index2D")
    int DX;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="NevarokML|Index2D")
    int DY;
    
    FNevarokMLIndex2D() : DX(-1), DY(-1)
    {
    }

    FNevarokMLIndex2D(const int x, const int y) : DX(x), DY(y)
    {
    }

    bool GreaterOrEqualZero() const { return DX >= 0 && DY >= 0; }

    static FNevarokMLIndex2D NegativeOne() { return FNevarokMLIndex2D(-1, -1); }

    FORCEINLINE FString ToString() const
    {
        return FString::Printf(TEXT("(DX=%d,DY=%d)"), DX, DY);
    }
};
