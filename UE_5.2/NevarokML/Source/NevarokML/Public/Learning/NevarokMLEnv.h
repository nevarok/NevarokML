...
#include "NevarokMLEnv.generated.h"

UCLASS(Blueprintable)
class NEVAROKML_API ANevarokMLEnv : public AActor
{
    GENERATED_BODY()

protected:
    UPROPERTY(BlueprintReadWrite, Category = "NevarokML|Env")
    UNevarokMLSample* _actionSample;

    UPROPERTY(BlueprintReadWrite, Category = "NevarokML|Env")
    UNevarokMLSample* _observationSample;

    UPROPERTY(BlueprintReadWrite, Category = "NevarokML|Env")
    float _reward;

    UPROPERTY(BlueprintReadOnly, Category = "NevarokML|Env")
    float _episodeReward;

    UPROPERTY(BlueprintReadOnly, Category = "NevarokML|Env")
    float _maxEpisodeReward;
    ...
    void OnInit(UNevarokMLSpace* actionSpace, UNevarokMLSpace* observationSpace, bool simulate);

    UFUNCTION(BlueprintNativeEvent, Category = "NevarokML|Env")
    void OnStep(bool simulate);
    
    UFUNCTION(BlueprintNativeEvent, Category = "NevarokML|Env")
    void OnStepSkip(bool simulate);

    UFUNCTION(BlueprintNativeEvent, Category = "NevarokML|Env")
    void OnReset(bool simulate);
    ...
    UFUNCTION(BlueprintCallable, Category = "NevarokML|Env")
    void AddReward(const float value) { _reward += value; }

public:
    void ExecuteInit(UNevarokMLSpace* actionSpace, UNevarokMLSpace* observationSpace, bool simulate);
    void ExecuteStep(bool simulate);
    void ExecuteStepSkip(bool simulate);
    void ExecuteReset(bool simulate);

    UFUNCTION(BlueprintPure, Category = "NevarokML|Env")
    UNevarokMLSample* GetActionSample() const { return _actionSample; }

    UFUNCTION(BlueprintPure, Category = "NevarokML|Env")
    UNevarokMLSample* GetObservationSample() const { return _observationSample; }
    ...
    UFUNCTION(BlueprintPure, Category = "NevarokML|Env")
    int GetEpisodes() const { return _episodes; }

    UFUNCTION(BlueprintPure, Category = "NevarokML|Env")
    float GetEpisodeReward() const { return _episodeReward; }
};
