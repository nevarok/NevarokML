...
#include "NevarokMLEnvsSynchronizerComponent.generated.h"

UCLASS(ClassGroup=(Custom), meta=(BlueprintSpawnableComponent))
class NEVAROKML_API UNevarokMLEnvsSynchronizerComponent : public UActorComponent
{
    GENERATED_BODY()

private:
    UPROPERTY(EditAnywhere, Category="NevarokML|EnvsSynchronizerComponent", meta=(AllowPrivateAccess))
    TArray<ANevarokMLEnv*> _envs;

    UPROPERTY()
    TArray<ANevarokMLEnv*> _doneEnvs;

public:
    UNevarokMLEnvsSynchronizerComponent();

    UFUNCTION(BlueprintPure, Category="NevarokML|EnvsSynchronizerComponent")
    ...
    UFUNCTION(BlueprintPure, Category="NevarokML|EnvsSynchronizerComponent")
    bool IsWait(UPARAM(ref)const ANevarokMLEnv* env) const;

    UFUNCTION(BlueprintPure, Category="NevarokML|EnvsSynchronizerComponent")
    bool IsOneOrMoreDone() const;

    UFUNCTION(BlueprintCallable, Category="NevarokML|EnvsSynchronizerComponent")
    void SetDone(ANevarokMLEnv* env);

    UFUNCTION(BlueprintCallable, Category="NevarokML|EnvsSynchronizerComponent")
    void ResetDone();

    UFUNCTION(BlueprintCallable, Category="NevarokML|EnvsSynchronizerComponent")
    void AddEnv(ANevarokMLEnv* env);

    UFUNCTION(BlueprintCallable, Category="NevarokML|EnvsSynchronizerComponent")
    void RemoveEnv(ANevarokMLEnv* env);
};
