...
#include "NevarokMLHitSensorComponent.generated.h"

UCLASS(ClassGroup=(Custom), meta=(BlueprintSpawnableComponent))
class NEVAROKML_API UNevarokMLHitSensorComponent : public UActorComponent
{
    GENERATED_BODY()

private:
    UPROPERTY(EditAnywhere, Category="NevarokML|HitSensorComponent", meta=(AllowPrivateAccess))
    TArray<FName> _hitTags;

    UPROPERTY(EditAnywhere, Category="NevarokML|HitSensorComponent", meta=(AllowPrivateAccess))
    TArray<FName> _ignoreHitTags;
    
    UPROPERTY()
    TArray<FName> _allHitTags;
...
public:
    UNevarokMLHitSensorComponent();

    virtual void BeginPlay() override;

    UFUNCTION(BlueprintCallable, Category="NevarokML|HitSensorComponent")
    void ResetHits();

    UFUNCTION(BlueprintPure, Category="NevarokML|HitSensorComponent")
    bool ContainsHitTag(const FName tag) const;
