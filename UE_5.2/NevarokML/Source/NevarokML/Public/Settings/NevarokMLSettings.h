...
#include "NevarokMLSettings.generated.h"

UCLASS(config = EditorPerProjectUserSettings, meta = (DisplayName = "NevarokML"))
class NEVAROKML_API UNevarokMLSettings : public UDeveloperSettings
{
    GENERATED_BODY()

public:
    inline static const FString PLUGIN_NAME = "NevarokML";
    inline static const FString PLUGIN_VERSION = "0.9b";
    inline static const FString DEFAULT_IMPORT_FOLDER = "Import";
    inline static const FString DEFAULT_SAVE_FOLDER = "Save";
    inline static const FString DEFAULT_LOG_FOLDER = "Log";
    inline static const FString DEFAULT_SOURCE_FOLDER = "Source";
    inline static const FString DEFAULT_EXE_FOLDER = "NevarokMLExe";
    inline static const FString ARCHIVE_NAME = "NevarokMLExe.zip";
    inline static const FString EXE_NAME = TEXT("nevarok_ml.exe");

    FFilePath DefaultBackendExePath;
    FFilePath BackendArchivePath;
    FDirectoryPath BackendFolderPath;
    FDirectoryPath SourceFolderPath;
    
    UPROPERTY(config, EditAnywhere, Category = "Paths")
    FFilePath BackendExePath;
    
    UPROPERTY(config, EditAnywhere, Category = "Paths")
    FDirectoryPath SaveFolderPath;

    UPROPERTY(config, EditAnywhere, Category = "Paths")
    FDirectoryPath LogFolderPath;
    
    UPROPERTY(config, EditAnywhere, Category = "Debug")
    bool ForceDisplayWarnings = true;
    
    UPROPERTY(config, EditAnywhere, Category = "Debug")
    bool ForceDisplayErrors = true;

    UPROPERTY(config, EditAnywhere, Category = "Debug", meta=(ClampMin="0"))
    float DisplayDuration = 999.0;

    UNevarokMLSettings();
    
#if WITH_EDITOR
    virtual FText GetSectionDescription() const override { return FText::FromString("NevarokML Settings Panel"); }
#endif
};
