...
#include "Sockets.h"
#include "NevarokMLSocketServer.generated.h"

UCLASS(Blueprintable)
class NEVAROKML_API UNevarokMLSocketServer : public UObject
{
    GENERATED_BODY()

public:
    UNevarokMLSocketServer();
    virtual ~UNevarokMLSocketServer() override;

    UFUNCTION(BlueprintCallable, Category = "NevarokML|SocketServer")
    bool Initialize(const FString& address, const int32 port);
    ...
    UFUNCTION(BlueprintCallable, Category = "NevarokML|SocketServer")
    bool Receive(TArray<uint8>& data);
    ...
    UFUNCTION(BlueprintPure, Category = "NevarokML|SocketServer")
...
private:
    ...
    bool Send(FSocket* socket, const TArray<uint8>& data, bool& successful) const;
    bool Receive(FSocket* socket, TArray<uint8>& data, bool& successful) const;

    static bool IsConnected(FSocket* socket);
    bool SendAll(const TArray<uint8>& data);
};
