...
#include "NevarokMLLogUtils.generated.h"

UCLASS()
class NEVAROKML_API UNevarokMLLogUtils : public UBlueprintFunctionLibrary
{
    GENERATED_BODY()

public:
    UFUNCTION(BlueprintCallable, Category = "NevarokML|LogUtils")
    static void StartEval();

    UFUNCTION(BlueprintCallable, Category = "NevarokML|LogUtils")
    static void EndEval(const FString& prefix, bool display = false, float time = 2.0);
    
    UFUNCTION(BlueprintCallable, Category = "NevarokML|LogUtils")
    static void Log(const FString& msg, const bool display = false, const float time = 2.0);

    UFUNCTION(BlueprintCallable, Category = "NevarokML|LogUtils")
    static void LogError(const FString& msg, const bool display = false, const float time = 2.0);

    UFUNCTION(BlueprintCallable, Category = "NevarokML|LogUtils")
    static void LogWarning(const FString& msg, const bool display = false, const float time = 2.0);

    UFUNCTION(BlueprintCallable, Category = "NevarokML|LogUtils")
    static void DrawDebugString(AActor* owner, const FString& text, const FLinearColor textColor, const float duration, const FVector textLocation = FVector::ZeroVector);
};
