...
#include "NevarokMLStringUtils.generated.h"

UCLASS()
class NEVAROKML_API UNevarokMLStringUtils : public UBlueprintFunctionLibrary
{
    GENERATED_BODY()

public:
    UFUNCTION(BlueprintPure, Category = "NevarokML|StringUtils")
    static TArray<uint8> StringToBytes(const FString& text);

    UFUNCTION(BlueprintPure, Category = "NevarokML|StringUtils")
    static FString BytesToString(const TArray<uint8>& bytes);

    UFUNCTION(BlueprintPure, meta=(DisplayName = "To String (Index2D)", CompactNodeTitle = "->", BlueprintAutocast), Category="Utilities|String")
    static FString Conv_IndexToString(FNevarokMLIndex2D index);
};
