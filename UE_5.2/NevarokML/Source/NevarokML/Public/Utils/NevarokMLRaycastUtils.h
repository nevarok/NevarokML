...
#include "NevarokMLRaycastUtils.generated.h"
...
struct FNevarokMLRaycastData
{
    ...
    UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category="NevarokML|RaycastData")
    FHitResult Hit;
...
UCLASS()
class NEVAROKML_API UNevarokMLRaycastUtils : public UBlueprintFunctionLibrary
{
    GENERATED_BODY()

public:
    UFUNCTION(BlueprintCallable, Category = "NevarokML|RaycastUtils")
    static TArray<FNevarokMLRaycastData> RaycastRange(AActor* owner,
                                                      float vAngleRange,
                                                      float hAngleRange,
                                                      float vAngleOffset,
                                                      float hAngleOffset,
                                                      ...
                                                      float debugDrawTime);
    UFUNCTION(BlueprintCallable, Category = "NevarokML|RaycastUtils")
    static TArray<FNevarokMLRaycastData> RaycastHRange(AActor* owner,
                                                       float angleRange,
                                                       int num,
                                                       float length,
                                                       ETraceTypeQuery channel,
    ...
    UFUNCTION(BlueprintCallable, Category = "NevarokML|RaycastUtils")
    static TArray<FNevarokMLRaycastData> RaycastVRange(AActor* owner,
                                                       float angleRange,
                                                       int num,
                                                       float length,
                                                       ETraceTypeQuery channel,
                                                       ...
                                                       FLinearColor debugTraceHitColor,
                                                       float debugDrawTime);
    static FNevarokMLRaycastData Raycast(AActor* owner,
                                         const FRotator& rotation,
                                         const FVector& start,
                                         float length,
                                         ETraceTypeQuery channel,
                                         bool debug, FLinearColor debugTraceColor, FLinearColor debugTraceHitColor,
                                         float debugDrawTime);

    UFUNCTION(BlueprintCallable, Category = "NevarokML|RaycastUtils")
    static float GetAngleBetweenVectors(const FVector& aNormal, const FVector& bNormal);

    UFUNCTION(BlueprintCallable, Category = "NevarokML|RaycastUtils")
    static TArray<FNevarokMLRaycastData> RaycastFromTo(AActor* owner,
