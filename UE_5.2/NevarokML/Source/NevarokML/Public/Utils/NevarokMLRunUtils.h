...
#include "NevarokMLRunUtils.generated.h"

UCLASS()
class NEVAROKML_API UNevarokMLRunUtils : public UBlueprintFunctionLibrary
{
    GENERATED_BODY()

public:
    static FProcHandle RunExe(const FString& address, const int port, const int verbose);
    static bool RunPython(const FString& script,
                          const FString& activateBatPath,
                          const FString& environment,
                          const FString& address,
                          int port);
    static bool RunUnzip(const FString& from, const FString& to);
    
    static bool IsRunning(FProcHandle& handle) { return FPlatformProcess::IsProcRunning(handle); }
    static void Close(FProcHandle& handle) { return FPlatformProcess::CloseProc(handle); }
