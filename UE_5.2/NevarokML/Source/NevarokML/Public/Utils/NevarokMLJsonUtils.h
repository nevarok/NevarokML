...
#include "NevarokMLJsonUtils.generated.h"

UCLASS()
class NEVAROKML_API UNevarokMLJsonUtils : public UBlueprintFunctionLibrary
{
    GENERATED_BODY()

public:
    static void ToString(const TSharedPtr<FJsonObject>& jsonObject, FString& jsonString);
    static void ValueToString(const TSharedPtr<FJsonValue>& jsonValue, FString& jsonString);
    static bool TryGetJsonObject(const FString& jsonString, TSharedPtr<FJsonObject>& jsonObject);
    static TArray<uint8> ToBytes(const TSharedPtr<FJsonObject>& jsonObject);
};
