...
#include "NevarokMLSampleUtils.generated.h"

UCLASS()
class NEVAROKML_API UNevarokMLSampleUtils : public UBlueprintFunctionLibrary
{
    GENERATED_BODY()

public:
    UFUNCTION(BlueprintCallable, Category = "NevarokML|SampleUtils")
    static bool SetBoxVector(UPARAM(ref)UNevarokMLSample* sample, const FNevarokMLIndex2D x, const FNevarokMLIndex2D y,
                             const FNevarokMLIndex2D z, const FVector vector, bool multicast);

    UFUNCTION(BlueprintCallable, Category = "NevarokML|SampleUtils")
    static bool SetContinuousVector(UPARAM(ref)UNevarokMLSample* sample, const int x, const int y, const int z, const FVector vector, bool multicast);

    UFUNCTION(BlueprintPure, Category = "NevarokML|SampleUtils")
    static bool GetBoxVector(UPARAM(ref)UNevarokMLSample* sample, const FNevarokMLIndex2D& x, const FNevarokMLIndex2D& y,
                             const FNevarokMLIndex2D& z, FVector& vector);

    UFUNCTION(BlueprintPure, Category = "NevarokML|SampleUtils")
    static bool GetContinuousVector(UPARAM(ref)UNevarokMLSample* sample, int x, int y, int z, FVector& vector);

    UFUNCTION(BlueprintCallable, Category = "NevarokML|SampleUtils")
    static bool SetMultiDiscreteArray(UPARAM(ref)UNevarokMLSample* sample, int from, UPARAM(ref)const TArray<int64>& array, bool multicast);
};
