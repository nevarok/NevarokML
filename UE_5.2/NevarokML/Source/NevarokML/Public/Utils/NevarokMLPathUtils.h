...
#include "NevarokMLPathUtils.generated.h"

UCLASS()
class NEVAROKML_API UNevarokMLPathUtils : public UBlueprintFunctionLibrary
{
    GENERATED_BODY()

public:    
    UFUNCTION(BlueprintPure, Category = "NevarokML|PathUtils")
    static FString GetPythonScriptPath(const FString& relativePath);

    UFUNCTION(BlueprintPure, Category = "NevarokML|PathUtils")
    static FString GetContentDir();

    UFUNCTION(BlueprintPure, Category = "NevarokML|PathUtils")
    static FString GetFolderNameFromPath(const FString& path);
};
