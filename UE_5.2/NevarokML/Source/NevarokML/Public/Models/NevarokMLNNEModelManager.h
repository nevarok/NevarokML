...
class NEVAROKML_API FNevarokMLNNEModelManager
{
private:
    TMap<const UNNEModelData*, TSharedPtr<UE::NNECore::IModelCPU>> _models;

public:
    static FNevarokMLNNEModelManager& GetInstance();

    bool ContainsModel(const UNNEModelData* modelData) const;

    TSharedPtr<UE::NNECore::IModelCPU> GetModel(const UNNEModelData* modelData) const;

    void AddModel(const UNNEModelData* modelData, const TSharedPtr<UE::NNECore::IModelCPU>& model);

    TSharedPtr<UE::NNECore::IModelCPU> LoadModel(UNNEModelData* modelData, const UNevarokMLSpace* obsSpace);
    TSharedPtr<UE::NNECore::IModelCPU> CreateModelCPU(UNNEModelData* modelData) const;
    TWeakInterfacePtr<INNERuntimeCPU> GetRuntime(bool& valid) const;

    static bool IsValid(TSharedPtr<UE::NNECore::IModelCPU> model, const UNevarokMLSpace* obsSpace);

private:
    FNevarokMLNNEModelManager();
};